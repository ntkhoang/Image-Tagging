...
def evaluate_model_on_voc_vit(voc_dir, model_path=None, batch_size=16, 
                         device='cuda', max_images=None, image_size=448,
                         num_workers=2, visualize=False, split='test'):
    """
    Evaluate a trained ImageGCN model with ViT backbone on VOC 2007 test set
    ...
    Args:
        ...
        image_size: Size of images for evaluation (must match training size)
    ...
    print(f"Evaluating on {len(eval_dataset)} {split} images with image size {image_size}x{image_size}")
    
    # Initialize model with ViT backbone
    model = ImageGCNSimple(
        num_classes=NUM_CLASSES,
        feature_dim=768,  # ViT feature dimension is 768 (not 2048 like ResNet)
        hidden_dim=512,
        dropout=0.0,  # No dropout for evaluation
        use_vit=True,  # Force ViT usage
        device=device,
        img_size=image_size  # Use the same image size as during training
    )
    
    # Load model weights if provided
    if model_path:
        print(f"Loading ViT model weights from {model_path}")
        try:
    ...
    if visualize:
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f'voc_{split}_class_metrics_vit.png')
        print(f"Per-class metrics visualization saved to voc_{split}_class_metrics_vit.png")
        ...
        plt.savefig(f'voc_{split}_label_cooccurrence_vit.png')
        print(f"Label co-occurrence matrix saved to voc_{split}_label_cooccurrence_vit.png")
    
    return results

def main():
    parser = argparse.ArgumentParser(description='Evaluate ImageGCN model with ViT backbone on VOC 2007 dataset')
    ...
    parser.add_argument('--image-size', type=int, default=448,
                        help='Image size for evaluation (must match training size)')
    ...
    try:
        ...
        print("\nExample usage:")
        print("python evaluate_voc_ViT.py --voc-dir /path/to/VOCdevkit/VOC2007 --model-path ./models_vit/best_model_voc_vit.pth --split val --image-size 448")
