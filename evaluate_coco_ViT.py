...
def evaluate_model_on_coco_vit(coco_dir, model_path=None, batch_size=8, device='cuda', max_images=5000, image_size=448):
    """
    Evaluate ImageGCN model with ViT backbone on COCO dataset
    ...
    Args:
        ...
        image_size: Image size for evaluation (must match the size used for training)
    ...
    # Create transforms - make sure to use the same image size as during training
    ...
    model = ImageGCNSimple(
        ...
        feature_dim=768,  # ViT feature dim is 768 (not 2048 like ResNet)
        hidden_dim=512,
    ...
    # Initialize trainer for evaluation functions
    trainer = ImageGCNTrainer(model, device=device)
    
    print("Evaluating ViT model on COCO val2017 dataset...")
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate ImageGCN with ViT backbone on COCO dataset')
    ...
    try:
        ...
        print("\nExample usage:")
        print("python evaluate_coco_ViT.py --coco-dir /path/to/coco --model-path ./models_vit/best_model_coco_vit.pth --image-size 448")
